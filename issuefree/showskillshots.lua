--Shows path of enemy skillshots
--Also shows end location of target skillshots (Ex:enemy shaco decieve)
local cc = 0
...
function dodgeaoe(pos1, pos2, radius)
    ...
    if calc < radius and dodgeskillshot == true then
...
function dodgelinepoint(pos1, pos2, radius)
    ...
    if perpendicular < radius and calc1 < calc4 and calc2 < calc4 and dodgeskillshot == true then
    	player:MoveTo(dodgex,dodgez)
    end
end
...
function MyProcessSpell(unit, spell)
local player=GetSelf()
    ...
    if string.find(unit.name,"Minion_") == nil and string.find(unit.name,"Turret_") == nil then
        ...
        if (unit.team ~= player.team or (show_allies==1)) and string.find(spell.name,"Basic") == nil then
...
function sample_CallBackskill()
	...
	---DrawText("testingkey:" .. tostring(IsKeyDown(49)),20,(220+cc)%1000,0xFF44DD00)
	--DrawText("Cursor:" .. tostring(GetCursorX()) .. "," .. tostring(GetCursorY()), GetCursorX(), GetCursorY(), 0xFF778822)
	--LoadTable()
	--printtext(cc.index)
	DoSpells(MyProcessSpell)
	--DrawText(tostring(GetSelf().name=="Morgana") .. tostring(skillshotcharexist) .. table_print(skillshotArray),48,200,0xFF33DD00)
...
SetTimerCallback("sample_CallBackskill")

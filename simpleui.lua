...
local function layout_pop(increment)
    ...
    -- increment next current layout by popped layout
    local current = layout_peek()
...
local function do_label(id, text, rect)
...
local function do_handle(uiid, rect, value1, value2)
    ...
    elseif uistate.step == UIStep_Logic then
        ...
        if just_activating then
            activated_delta_x = uistate.mouselocation.X - rect.X
            activated_delta_y = uistate.mouselocation.Y - rect.Y
        end

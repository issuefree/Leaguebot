    ...
    -- v06 - 5/11/2013 11:17:57 AM - added block_input, requires winapi v04
    --
    ...
    function M.is_buffer_full()
        return #M._buffer>=M._buffer_max
    end
    ...
    function M.block_input(flag, timeout)
        --print('\nblock_input')
        local t
        if flag then
            if timeout==nil then timeout=10*1000 end
            t = {fn=M._begin_block_input, args={timeout}, countdown=0}        
        else
    ...
    function M._begin_block_input(timeout)
        print('\n_begin_block_input')
        local now = os.clock() -- used as an id
    ...
    -- only ends if id matches _input_blocked_at
    function M._timeout_block_input(id)
        print('\n_timeout_block_input')
        if id == M._input_blocked_at then
            winapi.block_input(false)
        end
    end
